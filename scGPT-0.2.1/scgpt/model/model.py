...
from torch.nn import TransformerEncoder, TransformerEncoderLayer, BatchNorm1d, ModuleDict
from torch.distributions import Bernoulli
from tqdm import trange

from flash_attn.flash_attention import FlashMHA, MultiFlashMHA
...
class TransformerModel(nn.Module):
    ...
    def forward(
        ...
        if self.use_batch_labels and self.use_mod:
            cat_0 = batch_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1) + mod_emb
...
class MultiTransformerModel(nn.Module):
    def __init__(
        ...
        if domain_spec_batchnorm:
            ...
            print("Using simple batchnorm instead of domain specific batchnorm")
            # self.bn = nn.BatchNorm1d(d_model, eps=6.1e-5)
            self.bn_modules = ModuleDict({
                key: BatchNorm1d(d_model, eps=6.1e-5) for key in lora_keys
            })
        ...
        if use_fast_transformer:
            if fast_transformer_backend == "linear":
                ...
                encoder_layers = MultiFlashTransformerEncoderLayer(
                    d_model,
                    nhead,
                    d_hid,
                    dropout,
                ...
                self.transformer_encoder = MultiTransformerEncoder(encoder_layers, nlayers)
        else:
            encoder_layers = TransformerEncoderLayer(
                d_model, nhead, d_hid, dropout, batch_first=True
            )
    ...
    def _encode(
        ...
        if self.domain_spec_batchnorm:
            ...
            # total_embs = self.bn(total_embs.permute(0, 2, 1)).permute(0, 2, 1)
            assert lora_key in self.bn_modules
            total_embs = self.bn_modules[lora_key](total_embs.permute(0, 2, 1)).permute(0, 2, 1)


        output = self.transformer_encoder(
            total_embs, src_key_padding_mask=src_key_padding_mask, lora_key=lora_key
        )
        return output  # (batch, seq_len, embsize)
    ...
    def forward(
        ...
        """
        transformer_output = self._encode(
            src, values, src_key_padding_mask, batch_labels, lora_key=lora_key
        )
        if self.use_batch_labels:
            batch_emb = self.batch_encoder(batch_labels)  # (batch, embsize)
...
# TODO
class MultiFlashTransformerEncoderLayer(nn.Module):
    r"""
    MultiFlashTransformerEncoderLayer: use MultiFlashMHA
    """
    __constants__ = ["batch_first"]
    ...
    def forward(
        ...
        """
        if src_mask is not None:
            raise ValueError("MultiFlashTransformerEncoderLayer does not support src_mask")
        ...
        if self.norm_scheme == "pre":
            src = self.norm1(src)
            src2 = self.self_attn(src, lora_key=lora_key,key_padding_mask=src_key_padding_mask_)[0]
            src = src + self.dropout1(src2)
            src = self.norm2(src)
            src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
            src = src + self.dropout2(src2)
...
class MultiTransformerEncoder(TransformerEncoder):
    def __init__(self, encoder_layer, num_layers, norm=None, enable_nested_tensor=True, mask_check=True):
        super(MultiTransformerEncoder, self).__init__(encoder_layer, num_layers, norm, enable_nested_tensor, mask_check)

    def forward(self, src: Tensor, lora_key: str, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        for mod in self.layers:
            output = mod(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask_for_layers, lora_key=lora_key)
