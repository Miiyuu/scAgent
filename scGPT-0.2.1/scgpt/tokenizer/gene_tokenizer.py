...
def tokenize_batch(
    ...
    for i in range(len(data)):
        ...
        if use_idx:
            values=row[gene_idx]
            genes=gene_ids[gene_idx]
        else:
            row2=row
            gene_ids2=gene_ids

            indices = np.where(row2 != -3)[0]
            row = row2[indices]
            gene_ids_ = gene_ids2[indices]
            if mod_type is not None:
                mod_type_ = mod_type[indices]
...
def pad_batch(
    ...
    for i in range(len(batch)):
        ...
        if len(gene_ids) > max_len:
            if use_idx:
                assert False, f"gene_ids length {len(gene_ids)} > max_len {max_len}"
        ...
        if len(gene_ids) < max_len:
            ...
            if not cls_appended:
                pass
                idx = np.random.choice(len(gene_ids), len(gene_ids), replace=False)
            else:
                idx = np.random.choice(len(gene_ids) - 1, len(gene_ids) - 1, replace=False)
                idx = idx + 1
                idx = np.insert(idx, 0, 0)
            gene_ids = gene_ids[idx]
            values = values[idx]
...
def tokenize_and_pad_batch(
    ...
    batch_padded = pad_batch(
        tokenized_data, max_len, vocab, pad_token, pad_value, cls_appended=append_cls, vocab_mod=vocab_mod,use_idx=use_idx
    )
    return batch_padded
